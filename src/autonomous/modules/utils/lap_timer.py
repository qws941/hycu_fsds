#!/usr/bin/env python3
"""
Lap Timer - 랩타임 측정 및 주행 통계 수집 노드.

목적:
    자율주행 차량의 랩타임을 측정하고 거리, 속도 등 주행 통계를 수집합니다.
    시작점은 첫 위치에서 자동 설정되며, 최소 50m 이동 후 시작점 통과 시 랩 완료.

발행 토픽:
    /lap/current_time (Float32): 현재 랩 경과 시간 (초)
    /lap/count (Int32): 완료된 랩 수
    /lap/distance (Float32): 총 주행 거리 (m)
    /lap/best_time (Float32): 최고 랩타임 (초)
    /lap/avg_speed (Float32): 평균 속도 (m/s)
    /lap/max_speed (Float32): 최고 속도 (m/s)
    /lap/hud (String): HUD 디스플레이용 포맷팅된 문자열

구독 토픽:
    /fsds/testing_only/odom (Odometry): 차량 위치 및 속도

사용법:
    python3 lap_timer.py
    또는 docker-compose --profile monitoring up
"""
import numpy as np
from math import sqrt
from typing import TYPE_CHECKING

# Lazy ROS imports - only needed when running as ROS node
if TYPE_CHECKING:
    import rospy
    from nav_msgs.msg import Odometry
    from std_msgs.msg import String, Float32, Int32


class LapTimer:
    def __init__(self):
        # Lazy import ROS dependencies
        import rospy
        from nav_msgs.msg import Odometry
        from std_msgs.msg import String, Float32, Int32
        
        self._rospy = rospy
        self._Odometry = Odometry
        self._String = String
        self._Float32 = Float32
        self._Int32 = Int32
        
        rospy.init_node('lap_timer')
        
        self.start_x = None
        self.start_y = None
        self.start_threshold = 40.0  # Increased for CompetitionTrack finish line tolerance
        self.min_lap_distance = 20.0
        
        self.lap_count = 0
        self.lap_times = []
        self.lap_start_time = None
        self.total_distance = 0.0
        self.last_x = None
        self.last_y = None
        self.current_speed = 0.0
        self.max_speed = 0.0
        self.away_from_start = False
        
        self.lap_count_pub = rospy.Publisher('/lap/count', self._Int32, queue_size=1, latch=True)
        self.lap_time_pub = rospy.Publisher('/lap/current_time', self._Float32, queue_size=1)
        self.best_lap_pub = rospy.Publisher('/lap/best_time', self._Float32, queue_size=1)
        self.distance_pub = rospy.Publisher('/lap/distance', self._Float32, queue_size=1)
        self.avg_speed_pub = rospy.Publisher('/lap/avg_speed', self._Float32, queue_size=1)
        self.max_speed_pub = rospy.Publisher('/lap/max_speed', self._Float32, queue_size=1)
        self.hud_pub = rospy.Publisher('/lap/hud', self._String, queue_size=1)
        
        rospy.Subscriber('/fsds/testing_only/odom', self._Odometry, self.odom_callback)
        
        rospy.loginfo("=" * 50)
        rospy.loginfo("Lap Timer Started")
        rospy.loginfo("Waiting for first position to set start/finish line...")
        rospy.loginfo("=" * 50)
    
    def odom_callback(self, msg):
        try:
            x = msg.pose.pose.position.x
            y = msg.pose.pose.position.y
            vx = msg.twist.twist.linear.x
            vy = msg.twist.twist.linear.y
            
            if not np.isfinite([x, y, vx, vy]).all():
                return
            
            self.current_speed = sqrt(vx**2 + vy**2)
        
            if self.current_speed > self.max_speed:
                self.max_speed = self.current_speed
            
            if self.start_x is None:
                self.start_x = x
                self.start_y = y
                self.last_x = x
                self.last_y = y
                self.lap_start_time = self._rospy.Time.now()
                self._rospy.loginfo(f"Start/Finish set at ({x:.1f}, {y:.1f})")
                return
            
            if self.last_x is not None:
                dx = x - self.last_x
                dy = y - self.last_y
                self.total_distance += sqrt(dx**2 + dy**2)
            
            self.last_x = x
            self.last_y = y
            
            dist_to_start = sqrt((x - self.start_x)**2 + (y - self.start_y)**2)
            
            if not self.away_from_start and dist_to_start > self.min_lap_distance:
                self.away_from_start = True
            
            if self.away_from_start and dist_to_start < self.start_threshold:
                self.complete_lap()
        except Exception as e:
            self._rospy.logwarn_throttle(5.0, f"Odom callback error: {e}")
    
    def complete_lap(self):
        now = self._rospy.Time.now()
        lap_time = (now - self.lap_start_time).to_sec()
        
        if lap_time < 5.0:
            return
        
        self.lap_count += 1
        self.lap_times.append(lap_time)
        
        best_time = min(self.lap_times)
        avg_speed = self.total_distance / lap_time if lap_time > 0 else 0
        
        self._rospy.loginfo("=" * 50)
        self._rospy.loginfo(f"LAP {self.lap_count} COMPLETE!")
        self._rospy.loginfo(f"Time: {lap_time:.2f}s")
        self._rospy.loginfo(f"Best: {best_time:.2f}s")
        self._rospy.loginfo(f"Avg Speed: {avg_speed:.1f} m/s")
        self._rospy.loginfo(f"Max Speed: {self.max_speed:.1f} m/s")
        self._rospy.loginfo("=" * 50)
        
        self.lap_start_time = now
        self.total_distance = 0.0
        self.max_speed = 0.0
        self.away_from_start = False
    
    def publish_telemetry(self):
        self.lap_count_pub.publish(self._Int32(self.lap_count))
        
        if self.lap_start_time:
            current_time = (self._rospy.Time.now() - self.lap_start_time).to_sec()
            self.lap_time_pub.publish(self._Float32(current_time))
        
        if self.lap_times:
            self.best_lap_pub.publish(self._Float32(min(self.lap_times)))
        
        self.distance_pub.publish(self._Float32(self.total_distance))
        self.max_speed_pub.publish(self._Float32(self.max_speed))
        
        if self.lap_start_time:
            current_time = (self._rospy.Time.now() - self.lap_start_time).to_sec()
            avg_speed = self.total_distance / current_time if current_time > 0 else 0
            self.avg_speed_pub.publish(self._Float32(avg_speed))
        
        hud = self.format_hud()
        self.hud_pub.publish(self._String(hud))
    
    def format_hud(self):
        if not self.lap_start_time:
            return "WAITING FOR START..."
        
        current_time = (self._rospy.Time.now() - self.lap_start_time).to_sec()
        best_time = min(self.lap_times) if self.lap_times else 0.0
        avg_speed = self.total_distance / current_time if current_time > 0 else 0
        
        lines = [
            "╔════════════════════════════╗",
            f"║  LAP: {self.lap_count:3d}                  ║",
            f"║  TIME: {current_time:6.1f}s             ║",
            f"║  BEST: {best_time:6.1f}s             ║",
            f"║  SPEED: {self.current_speed:5.1f} m/s          ║",
            f"║  AVG:   {avg_speed:5.1f} m/s          ║",
            f"║  DIST:  {self.total_distance:6.1f}m           ║",
            "╚════════════════════════════╝"
        ]
        return "\n".join(lines)
    
    def run(self):
        rate = self._rospy.Rate(10)
        while not self._rospy.is_shutdown():
            self.publish_telemetry()
            rate.sleep()


def main():
    import rospy
    try:
        timer = LapTimer()
        timer.run()
    except rospy.ROSInterruptException:
        pass


if __name__ == '__main__':
    main()
